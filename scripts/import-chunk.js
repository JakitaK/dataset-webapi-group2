    ...
    for (const movie of batch) {
      ...
      try {
        ...
        if (existingMovie.rows.length > 0) {
          // Movie already exists, use existing ID
          movieId = existingMovie.rows[0].movie_id;
          console.log(`Skipping duplicate: "${movie.title}" (${movie.release_year})`);
        } else {
          ...
          movieId = movieResult.rows[0].movie_id;
        }
        
        // Insert movie-genre relationships (only for new movies)
        if (existingMovie.rows.length === 0 && movie.genres) {
        ...
        // Insert movie-actor relationships (only for new movies)  
        if (existingMovie.rows.length === 0) {
